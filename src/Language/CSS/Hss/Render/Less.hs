...
instance Convertible Val LessVal where
  convert (Val flags v) = LessVal (Set.member FlagImportant flags) $ convert v

instance Convertible Val LessRawVal where
  convert (Val _ v) = convert v

instance Convertible RawVal LessRawVal where
  convert = \case
    ValVar   a -> LessVar a
    ValTxt   a -> LessTxt a
    ValApp t a -> LessApp t (convert <$> a)
